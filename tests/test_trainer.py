...
class TestBuildTrainer(absltest.TestCase):
    ...
    def test_loading_trainer(self):
        orig_config = yaml.safe_load(pathlib.Path('tests/config/cifar10_classifier.yaml').read_text())
        orig_config = ConfigDict(orig_config)

        ckpt_folder = sorted(glob('tests/checkpoints/BuildTrainerTest/checkpoints/*'))[-1]
        trainer = ImgClassifierTrainer.load_from_checkpoint(ckpt_folder,
                                                            exmp_input_file='tests/checkpoints/BuildTrainerTest/exmp_input.pkl')
        
        self.assertTrue(trainer.trainer_config.logger.log_dir == orig_config.trainer.logger.log_dir)
