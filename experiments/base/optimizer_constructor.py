...
def build_optimizer(
    optimizer_config: ConfigDict, num_epochs: int = 0, num_train_steps_per_epoch: int = 0
):
    ...
    if optimizer_config.get("grad_clip_norm", None) is not None:
        grad_trans.append(optax.clip_by_global_norm(optimizer_config.grad_clip_norm))
    if optimizer_config.get("grad_clip_value", None) is not None:
        grad_trans.append(optax.clip(optimizer_config.grad_clip_value))
    if optimizer_config.get("weight_decay", 0.0) > 0.0 and optimizer_name not in ["adamw"]:
        grad_trans.append(optax.add_decayed_weights(optimizer_config.weight_decay))

    # Put everything together
    optimizer = optax.chain(*grad_trans, opt_class(lr_schedule))
