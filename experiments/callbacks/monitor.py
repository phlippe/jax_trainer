...
class LearningRateMonitor(Callback):
    ...
    def _on_training_epoch_start(self, epoch_idx):
        ...
        if schedule is None:
            ...
            return
        opt_state = [s for s in self.trainer.state.opt_state[-1] if isinstance(s, optax.ScaleByScheduleState)]
        if len(opt_state) == 0:
            logging.warning('No state of a learning rate schedule found.')
            return
        if len(opt_state) > 1:
            logging.warning('Found multiple states of a learning rate schedule. Using the last one.')
